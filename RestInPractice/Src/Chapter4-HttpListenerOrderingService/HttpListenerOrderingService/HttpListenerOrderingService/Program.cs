...
namespace Restbucks 
{
    public class Program
    {
        ...
        private static void CreateResource(HttpListenerContext context)
        {
            string orderId = "1234";
            Order order = new Order();
            var inputStream = context.Request.InputStream;
            var encodeing = context.Request.ContentEncoding;
            StreamReader sr = new StreamReader(inputStream, encodeing);
            string request = sr.ReadToEnd();
            sr.Close();
            

            OrderDatabase.Database.Save(orderId, order);
        }
        ...
        public static void DispatchRequest()
        {
            ...
            switch (contexts[Thread.CurrentThread].Request.HttpMethod.ToLower())
            {
                ...
                case "put":
                    {
                        Put(contexts[Thread.CurrentThread]);
                        break;
                    }
                case "delete":
                    {
